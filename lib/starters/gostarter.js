...
// let flag: any;
let goexe = which.sync('go', { nothrow: true });
let flag = {};
let func = {};
let h = new helpers_1.default();
async function goStarter(bot, ctx, conf) {
    try {
        let strt = new RegExp("^\\" + config_1.default.startSymbol + "(go|golang)", "i").test(ctx.message.text);
        let id = ctx.message.from.id;
        let cmp = "go";
        let msg = ctx.message;
        msg.text = msg.text.replace(/\/(go|golang)/i, "").trim();
        // if first time
        if (!func[id]) {
            func[id] = {};
        ...
        if (!(msg.text + "") && !msg.reply_to_message) {
            ...
            return replyy(ctx, "Enter go code " + ctx.from.first_name + ":");
        }
