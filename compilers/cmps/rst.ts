...
let rstyoyorst = async (bot: Telegraf, ctx: any, obj: Opt = {}) => {
  // obj = obj || {}
...
/*
    ...
    */
    ...
    fs.writeFileSync(`./files/rst/rst${fromId}rst.rs`, code);

    const { status, stderr } = spawnSync(config.rust, [`./files/rst/rst${fromId}rst.rs`, '-o', `./files/rst/rst${fromId}rst` ] );
    ...
    if (status != 0) {
      terminate(false)
      reply(stderr.toString().replace(/\.\/files\/rst\/rst\d+rst.rs/g, "See"))
      return ctx.scene.leave()
    ...
    ccode = spawn(`./files/rst/rst${fromId}rst`, [], {
    ...
    ccode.stderr.on('data', async (data: any) => {
      ...
      dstr = dstr.replace(/\.\/files\/rst\/rst\d+rst\.rs/g, "See")
      if (mid == 0 && m) {
        m = false
...
let terminate = async (slow: any = true) => {
  ...
  try {
    if (fs.existsSync(`./files/rst/rst${fromId}rst`)) {
      fs.unlinkSync(`./files/rst/rst${fromId}rst`);
    }
  } catch (err: any) { }
