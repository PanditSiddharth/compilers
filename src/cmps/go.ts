...
let goyoyogo = async (ctx: any, obj: any = {}) => {
  // obj = obj || {}
  ...
  try {
    ...
    newObj.goProjectDir = path.join('.', 'files', 'golang', `go${newObj.fromId}go`);
    newObj.goFilePath = path.join(newObj.goProjectDir, 'main.go');
    ...
    newObj.golang = spawn(newObj.goexe, ['run', newObj.goFilePath], {
      env: {
        GOCACHE: path.join(process.cwd(), newObj.goProjectDir),
      },
    });
    ...
    let m = true
    newObj.golang.stderr.on('data', async (data: any) => {
      if (newObj.mid == 0 && m) {
        m = false
    ...
    newObj.code = ""
    newObj.golang.on("error", (err: any) => { console.error(err); terminate(ctx, obj); })
    newObj.golang.on('close', function (statusCode: any) {
