...
let tsxpath = path.join('.', 'node_modules', '.bin', 'tsx')
let nodepath = path.dirname(jsexe);
let h = new Hlp();
...
let jsyoyojs = async (ctx: any, obj: any = {}) => {
  ...
  try {
    ...
    newObj.node = spawn(tsxpath, ['-e', newObj.code], config.spawnOptions ? config.spawnOptions : { env: {
      PATH: nodepath
    } });
